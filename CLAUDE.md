...
## 转会市场/经济(关键规则)
- **合约即状态**:长期合约/短期合约就是签约状态(已并入 `CarStatus`,无独立 `contract` 字段)。长期=转会窗口不释放、锁死原队;短期=可被挖角/进队时解约;自由市场签下默认短期。
...
- 转会为**半自动选秀**(2026-07-02 重构,见该日 spec;服务在 `market_draft.py`):**不再一刀切释放**。按「假设各队维持现有阵容」的**余额**排序(平局:车队积分→最高 MMR 车→固定种子随机),**逐队处理**;每确认一队后重算队列。进当前队即**自动解约自家全部短期**;**含长期车的类别必须先锁**;每类别用两下拉手动选(长期车钉死一槽),候选池含自由身 + **未确认队的短期车(挖角)**,建议(保持/解散/补强)仅供参考。**逃生阀**(无合法可负担车→含长期锁 1、无长期锁 0)保证不死锁;两个长期类别争预算由玩家取舍。每类别 **0 或 2 现役**;退役车不入市、不计薪资、不占名额。`MarketDraft`/`DraftCarSnapshot` 存草稿状态,支持锁定集合与**一键重置**。
